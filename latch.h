...
void _LATCH(const int start, const int thread_stride, const uint8_t* const __restrict image, const int stride, std::vector<cv::KeyPoint>& keypoints, uint64_t* const __restrict descriptors) {
	for (int i = start; i < start + thread_stride; ++i) {
		uint8_t* const __restrict desc = reinterpret_cast<uint8_t*>(descriptors + (i << 3));
		const cv::KeyPoint kp = keypoints[i];
		float angle =  kp.angle * deg2rad; 
		const __m128 ptx = _mm_set_ps1(kp.pt.x), pty = _mm_set_ps1(kp.pt.y);
		const __m128 sin_theta = _mm_set_ps1(sin(angle)), cos_theta = _mm_set_ps1(cos(angle));
		const __m128 scale = _mm_mul_ps(_mm_set_ps1(kp.size), _mm_set_ps1(0.142857142857f));
...
void LATCH(const uint8_t* const __restrict image, const int width, const int height, const int stride, std::vector<cv::KeyPoint>& keypoints, uint64_t* const __restrict descriptors) {
...
void _LATCH(const int start, const int thread_stride, const uint8_t* const __restrict image, const int stride, std::vector<Latch::KeyPoint>& keypoints, uint64_t* const __restrict descriptors) {
	for (int i = start; i < start + thread_stride; ++i) {
		uint8_t* const __restrict desc = reinterpret_cast<uint8_t*>(descriptors + (i << 3));
		const Latch::KeyPoint pt = keypoints[i];
...
template<bool multithread>
void LATCH(const uint8_t* const __restrict image, const int width, const int height, const int stride, std::vector<Latch::KeyPoint>& keypoints, uint64_t* const __restrict descriptors) {
	keypoints.erase(std::remove_if(keypoints.begin(), keypoints.end(), [width, height](const Latch::KeyPoint& kp) {return kp.x <= 36 || kp.y <= 36 || kp.x >= width - 36 || kp.y >= height - 36; }), keypoints.end());
	const int sz = static_cast<int>(keypoints.size());
	if (multithread) {
		const int32_t hw_concur = std::min(sz >> 4, static_cast<int32_t>(std::thread::hardware_concurrency()));
		if (hw_concur > 1) {
